...
<script>
    ...
    $(document).on("click", ".create-api-url", function () {
        let serviceKey = `ServiceKey=PNYIG396ZLS92MWQ4730`;
        let title = `title=\${document.querySelector(".title").value.trim() || ""}`;
        let releaseDte = `releaseDte=\${document.querySelector(".releaseDte").value.trim() || ""}`;
        let releaseDts = `releaseDts=\${document.querySelector(".releaseDts").value.trim() || ""}`;
        let listCount = `listCount=\${document.querySelector(".listCount").value.trim() || 3}`;
        let query = [title, releaseDte, releaseDts, serviceKey, listCount].join("&");

        let requestUrl = `http://api.koreafilm.or.kr/openapi-data2/wisenut/search_api/search_json2.jsp?collection=kmdb_new2&detail=Y&\${query}`;
